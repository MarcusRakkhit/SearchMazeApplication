...
namespace TheRobotNavigationProblem
{
    internal class BFS
    {
        ...
        public BFS(List<Node> newMap, Agent newAgent) 
        { 
            map = newMap;
            agent = newAgent;
            InitialiseStack();
        ...
        public State? Search()
        {
            ...
            while (stack.Count > 0)
            {
                ...
                {
                    result = currentState;
                    Console.WriteLine("Solution Found with 'BFS SEARCH'-");
                ...
                //Adds each move
                //In BFS, all new nodes are added to the end of the list to allow the search to search from left - right in the tree branch
