...
namespace TheRobotNavigationProblem
{
    internal class AStar
    {
        ...
        public AStar(List<Node> newMap, Agent newAgent)
        {
            map = newMap;
            agent = newAgent;
            InitialiseStack();
        ...
        public State? Search()
        {
            ...
            while (stack.Count > 0)
            {
                //Takes shortest path by AStar heuristics from stack and removes it
                State currentState = calculations.SelectedShortestAstar(stack);
                ...
                {
                    result = currentState;
                    Console.WriteLine("Solution Found with 'A* SEARCH'-");
